...
!function ($) {
	...
	var Datetimepicker = function (element, options) {
		...
		this.component = this.element.is('.date') ? this.element.find('.input-group-addon .icon-time, .input-group-addon .glyphicon-time, .input-group-addon .icon-calendar').parent() : false;
	...
	Datetimepicker.prototype = {
		...
		fill: function () {
			...
			if (this.formatViewType == "time") {
				...
				this.picker.find('.datetimepicker-hours thead th:eq(1)')
					.text(dayMonth + ' ' + moment.langData().months(moment().month(month)) + ' ' + year);
				this.picker.find('.datetimepicker-minutes thead th:eq(1)')
					.text(dayMonth + ' ' + moment.langData().months(moment().month(month)) + ' ' + year);
			}
			this.updateNavArrows();
